...
namespace QLBH
{
    public partial class frmKhachHang : Form
    {
        ...
        {
            var sql = @"SELECT MaKH, HoVaTen, DiaChi, GioiTinh, SoDienThoai, 
                            NgaySinh = CONVERT(NVARCHAR(10),NgaySinh,103), Email 
                        FROM KhachHang";
        ...
        private void btnThemKH_Click(object sender, EventArgs e)
        {
            try
            {
                var sql = @"INSERT INTO KhachHang (MaKH, HoVaTen, DiaChi, GioiTinh, SoDienThoai, NgaySinh, Email) 
                            VALUES (@MaKH, @HoVaTen, @DiaChi, @GioiTinh, @SoDienThoai, @NgaySinh, @Email)";

                var cmd = new SqlCommand(sql, DBConnect.Connect());
                cmd.Parameters.AddWithValue("MaKH", txtMaKH.Text.Trim());
                cmd.Parameters.AddWithValue("HoVaTen", txtHoTenKH.Text.Trim());
                cmd.Parameters.AddWithValue("DiaChi", txtDiaChi.Text.Trim());
                cmd.Parameters.AddWithValue("GioiTinh", rdNam.Checked ? "Nam" : "Nữ");
                cmd.Parameters.AddWithValue("SoDienThoai", txtSDTKH.Text.Trim());
                cmd.Parameters.AddWithValue("NgaySinh", dateKH.Value);
                cmd.Parameters.AddWithValue("Email", txtEmailKH.Text.Trim());
        ...
        private void btnSuaKH_Click(object sender, EventArgs e)
        {
            try
            {
                var sql = @"UPDATE KhachHang SET HoVaTen = @HoVaTen, DiaChi = @DiaChi, GioiTinh = @GioiTinh, 
