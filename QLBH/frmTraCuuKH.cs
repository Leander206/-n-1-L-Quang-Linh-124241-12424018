...
namespace QLBH
{
    public partial class frmTraCuuKH : Form
    {
        ...
        private void TimKiem()
        {
            try
            {
                var sql = "SELECT MaKH, HoVaTen, TenDangNhap, MatKhau, DiaChi, GioiTinh, SoDienThoai, NgaySinh = CONVERT(NVARCHAR(10), NgaySinh, 103), Email FROM KhachHang WHERE MaKH LIKE N'%' + @TuKhoa + '%' OR HoVaTen LIKE N'%' + @TuKhoa + '%' OR TenDangNhap LIKE N'%' + @TuKhoa + '%' OR MatKhau LIKE N'%' + @TuKhoa + '%' OR DiaChi LIKE N'%' + @TuKhoa + '%' OR GioiTinh LIKE N'%' + @TuKhoa + '%' OR SoDienThoai LIKE N'%' + @TuKhoa + '%' OR CONVERT(NVARCHAR(10), NgaySinh, 103) LIKE N'%' + @TuKhoa + '%' OR Email LIKE N'%' + @TuKhoa + '%'";
